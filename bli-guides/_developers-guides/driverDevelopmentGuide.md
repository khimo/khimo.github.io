...
The driver must implement a corresponding [query](#query) function to handle these queries and return the appropriate data.

Capabilities that include queries (such as INPUT, APPLICATION, CHANNEL, CUSTOM\_COMMAND and PLAYQUEUE) automatically add the corresponding query specification when using the resources abstraction library. See the [RENDERER type](#RENDERER-type) section for the expected response formats.
